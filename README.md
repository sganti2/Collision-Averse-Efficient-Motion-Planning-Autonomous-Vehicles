...
Frenet frames were utilized as a baseline for validating the computational efficiency of the extended motion planner. However, the Frenet frame approach does not incorporate minimization, or take the vehicle dynamics into account when generating the desired trajectory. In order to visualize the Frenet frame results, execute the 'frenet_optimal_trajectory.py' file within the specified directory. Ensure that the 'cubic_spline_planner.py' and 'quintic_polynomials_planner.py' files are included within the same package as the 'frenet_optimal_trajectory.py' while executing the source code. 
